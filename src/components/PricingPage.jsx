  ...
  return (
    <div className={`rounded-2xl border ${highlight ? 'border-blue-500/60' : 'border-slate-800'} bg-slate-900/40 p-6 flex flex-col`}>
      <div className="flex items-baseline justify-between">
        ...
        {highlight && (
          <span className="text-xs px-2 py-1 rounded-full bg-blue-500/10 text-blue-300 border border-blue-500/30">Most Popular</span>
        )}
      </div>
      <div className="mt-4">
        <div className="text-4xl font-bold">
          {price}
          <span className="text-lg font-medium text-slate-400">/{period}</span>
        </div>
      ...
      <div className="mt-6 grid grid-cols-1 gap-3">
        <a
          ...
          className={`inline-flex items-center justify-center px-4 py-2 rounded-md font-semibold ${highlight ? 'bg-white text-slate-900' : 'bg-slate-800 text-white hover:bg-slate-700'} transition-colors`}
        >
          {ctaLabel}
        </a>
        <a
          href="https://calendly.com/datagen-ai/demo"
          target="_blank"
          rel="noopener noreferrer"
          className="inline-flex items-center justify-center px-4 py-2 rounded-md font-semibold border border-slate-700 text-slate-200 hover:bg-slate-800 transition-colors"
        >
          Book a live demo
        </a>
      </div>
  ...
  const tiers = [
    {
      name: 'Starter',
      price: '$0',
      bullets: ['Up to 2 datasets', 'Auto-generated dashboards', 'Basic insights', 'Email support'],
    ...
    },
    {
      name: 'Pro',
      price: '$49',
      bullets: ['Unlimited datasets', 'Advanced insights & alerts', 'Team collaboration', 'Priority support'],
      highlight: true,
    ...
    },
    {
      name: 'Enterprise',
      price: 'Custom',
      bullets: ['SSO & advanced security', 'VPC / on-prem options', 'Custom SLAs', 'Dedicated support'],
